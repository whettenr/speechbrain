...
if __name__ == "__main__":
    ...
    print('weights before loading ckpt')
    print(asr_brain.modules.enc.transformer.encoder.layers[5].mha_layer.linear_pos.weight.view(-1)[:10])
    ...
    print('weights after loading ckpt')
    print(asr_brain.modules.enc.transformer.encoder.layers[5].mha_layer.linear_pos.weight.view(-1)[:10])

    pt_ckpts = hparams["checkpointer_pt_model"].list_checkpoints()
    print(f"averaging {len(pt_ckpts)} ckpts...")
    avg_ckpt_model = average_checkpoints(pt_ckpts, "model")
    print(f"done averaging {len(pt_ckpts)} ckpts")
    print('weights after afterging')
    asr_brain.modules.pt_model.load_state_dict(avg_ckpt_model)
    print(asr_brain.modules.enc.transformer.encoder.layers[5].mha_layer.linear_pos.weight.view(-1)[:10])
    exit()
